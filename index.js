...
console.log(chalk.whiteBright('          🚀 Bienvenido al núcleo de la Bot Luffy 🚀'))
console.log(chalk.whiteBright('    Prepara tu sesión... Luffy no puede esperar para servirte querido usuario ✨'))
console.log(chalk.magentaBright('═════════════════════════════════════════════════════════════════════\n'))
...
const {state, saveState, saveCreds} = await useMultiFileAuthState(global.Luffysessions)
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${Luffysessions}/creds.json`)) {
  do {
    opcion = await question(colores('⌨ Seleccione una opción:\n') + opcionQR('1. Con código QR\n') + opcionTexto('2. Con código de texto de 8 dígitos\n--> '))

    if (!/^[1-2]$/.test(opcion)) {
      console.log(chalk.bold.redBright(`✦ No se permiten numeros que no sean 1 o 2, tampoco letras o símbolos especiales.`))
    }} while (opcion !== '1' && opcion !== '2' || fs.existsSync(`./${Luffysessions}/creds.json`))
} 
...
if (!fs.existsSync(`./${Luffysessions}/creds.json`)) {
...
async function connectionUpdate(update) {
  ...
  if (connection == 'open') {
    console.log(chalk.bold.green('\n❀ Luffy-Bot Conectada con éxito ❀'))
  }
  let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
  if (connection === 'close') {
    if (reason === DisconnectReason.badSession) {
      console.log(chalk.bold.cyanBright(`\n⚠︎ SIN CONEXIÓN, BORRE LA CARPETA ${global.Luffysessions} Y ESCANEA EL CÓDIGO QR ⚠︎`))
  ...
  if (readRutaJadiBot.length > 0) {
    ...
    for (const gjbts of readRutaJadiBot) {
      ...
      if (readBotPath.includes(creds)) {
        blackJadiBot({pathRubyJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot'})
      }
    }
  }
}
...
function purgeOldFiles() {
  const directories = [`./${Luffysessions}/`, `./${jadi}/`]
