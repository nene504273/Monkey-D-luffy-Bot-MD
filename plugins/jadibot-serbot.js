...
let pathLuffyJadiBot = path.join(`./${jadi}/`, id) // Renombrado de pathMariaJadiBot a pathLuffyJadiBot
if (!fs.existsSync(pathLuffyJadiBot)){
fs.mkdirSync(pathLuffyJadiBot, { recursive: true })
}
LuffyJBOptions.pathLuffyJadiBot = pathLuffyJadiBot // Actualizada la variable dentro del objeto
LuffyJBOptions.m = m
...
const pathCreds = path.join(pathLuffyJadiBot, "creds.json") // Usa pathLuffyJadiBot
if (!fs.existsSync(pathLuffyJadiBot)){
fs.mkdirSync(pathLuffyJadiBot, { recursive: true })} // Usa pathLuffyJadiBot
try {
...
const { state, saveState, saveCreds } = await useMultiFileAuthState(pathLuffyJadiBot) // Usa pathLuffyJadiBot
...
userJid = sock.authState.creds.me.jid || `${path.basename(pathLuffyJadiBot)}@s.whatsapp.net` // Usa pathLuffyJadiBot
console.log(chalk.bold.cyanBright(`\n❒⸺⸺⸺⸺【• SUB-BOT •】⸺⸺⸺⸺❒\n│\n│ 🟢 ${userName} (+${path.basename(pathLuffyJadiBot)}) conectado exitosamente.\n│\n❒⸺⸺⸺【• CONECTADO •】⸺⸺⸺❒`)) // Usa pathLuffyJadiBot
sock.isInit = true
